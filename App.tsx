...
import { UndoIcon, RedoIcon, EyeIcon, MagicWandIcon, CropIcon, SlidersIcon, PaletteIcon, SparklesIcon } from './components/icons';
...
const TABS: Tab[] = ['retouch', 'crop', 'adjust', 'filters', 'generate'];

const tabConfig: { [key in Tab]: { icon: React.FC<{className?: string}>, label: string } } = {
    retouch: { icon: MagicWandIcon, label: 'Retouch' },
    crop: { icon: CropIcon, label: 'Crop' },
    adjust: { icon: SlidersIcon, label: 'Adjust' },
    filters: { icon: PaletteIcon, label: 'Filters' },
    generate: { icon: SparklesIcon, label: 'Generate' },
};
...
const App: React.FC = () => {
  ...
  const handleGenerateImageFromText = useCallback(async (generationPrompt: string) => {
    if (history.length > 0) {
        const userConfirmed = window.confirm(
            "Generating a new image will replace your current image and all its edits. Are you sure you want to continue?"
        );
        if (!userConfirmed) {
            return;
        }
    ...
    try {
        ...
        // This is a new base image, so reset the history
        ...
        setEditHotspot(null);
        setDisplayHotspot(null);
        // Switch to a more useful tab after generation
        setActiveTab('retouch');
  ...
  const renderContent = () => {
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-stretch justify-center gap-2 backdrop-blur-sm">
          {TABS.map(tab => {
              ...
              return (
                 <button
                    ...
                    className={`w-full flex flex-col sm:flex-row items-center justify-center gap-2 capitalize font-semibold py-3 px-4 rounded-md transition-all duration-200 text-base ${
                        activeTab === tab 
                        ? (isGenerate ? 'bg-gradient-to-br from-purple-500 to-indigo-400 text-white shadow-lg shadow-indigo-500/40' : 'bg-gradient-to-br from-blue-500 to-cyan-400 text-white shadow-lg shadow-cyan-500/40')
        ...
        <div className="w-full">
            ...
            {activeTab === 'generate' && <GeneratePanel onGenerateImage={handleGenerateImageFromText} isLoading={isLoading} />}
        </div>
